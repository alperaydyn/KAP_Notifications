...
class reader():
    ...
    def has_hidden_parent(self, e):
        ...
        if 'display: none' in pstyle:
            ...
            return self.has_hidden_parent(p)
    ...
    def read_company_detail(self, url, lang='tr'):
        ...
        sector = ''
        try:
            sector = pdiv.find_all("div", string=sector_text)[0].findNext("div", {"class": "exportDiv"}).text.strip()
        except:
            pass
    ...
    def get_notification(self, id, lang='tr', return_soup=False):
        ...
        try:
            req = requests.get(f"https://www.kap.org.tr/{lang}/Bildirim/{id}", headers=self.headers,
        ...
        if req.status_code != 200:
            ...
            # code ----------------------------------------------------------------------
            code = soup.find_all('div', {'class': 'modal-headertext'})[0].find_all('div', {
                'class': 'type-medium bi-dim-gray'})[0].text
    ...
    def _refresh_all_notifications(self, start=None, stop=None):
        ...
        kdf = self.db.read_data(sql='select min(NOTIFICATION_ID), min(PUBLISH_DATE), count(*) from KAP_NOTIFICATIONS')[
            0]
        print(kdf)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--read_notifs", type=int,
                        help="read new notifications from web starting from max to max+limit (reader.get_notifications(limit={limit})). -1 for no limit",
                        default=0)
